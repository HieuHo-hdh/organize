...
function* getOrganizeList({ payload: {params} }){
    ...
    if (params.search)
    {
        if (params.search.organizeName)
            searchParams.organizeName = params.search.organizeName;
    }
    ...
    try {
        ...
        yield put ({
            ...
            organizeListData: result.responseData.data && result.responseData.data
        })
    }
    catch(error) {
...
const sagas = [
    ...
    takeLatest(defineActionLoading(SEARCH_ORGANIZE), searchOrganize),
    takeLatest(define(GET_ORGANIZE), getOrganize),
    takeLatest(define(CREATE_ORGANIZE), createOrganize),
    takeLatest(define(UPDATE_ORGANIZE), updateOrganize),
    takeLatest(define(DELETE_ORGANIZE), deleteOrganize),
]
